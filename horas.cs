...
namespace AgendamentoServicos
{
    public partial class Horarios : Form
    {
        ...
        {
            ComboBox[] comboBoxes = { comboBoxSunday, comboBoxMonday, comboBoxTuesday, comboBoxWednesday, comboBoxThursday, comboBoxFriday, comboBoxSaturday };
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                {
                    foreach (string horario in horariosPorDia[hora, i])
                    {
                        if (!comboBoxes[i].Items.Contains(horario)) // Verifica se o horário já foi adicionado
                        {
                            comboBoxes[i].Items.Add(horario);
                        }
                    }
                }
            }
